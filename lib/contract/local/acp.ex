defmodule Contract.Local.ACP do
  @moduledoc """
  ...
  alias Contract.Local.Agent.Adapters.ClaudeCLI
  alias Contract.Local.Agent.Adapters.CodexAppServer
  alias Contract.Local.Agent.Adapters.Fake
  alias Contract.Local.Agent.Adapters.Unavailable
  alias Contract.Local.Agent.Session
  alias Contract.Local.Agent.SessionSupervisor
  ...
  def subscribe(session_id) when is_binary(session_id) do
    Phoenix.PubSub.subscribe(Contract.PubSub, topic(session_id))
  end
  ...
  defp start_registered_server do
    case Process.whereis(Contract.Supervisor) do
      pid when is_pid(pid) ->
        case Supervisor.start_child(Contract.Supervisor, __MODULE__) do
  ...
  defp validate_fake_adapter_override do
    if Application.get_env(:contract, :env) == :test do
      :ok
    else
