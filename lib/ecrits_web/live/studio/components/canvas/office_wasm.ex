    ...
    >
      ...
      <script :type={Phoenix.LiveView.ColocatedHook} name=".WasmOfficeEditor">
        ...
        const opSetCell = (ctx, op) => {
          ...
          return { op: { op: "set_text", ref: hit.targetRef, text: String(op.text == null ? "" : op.text) } };
        };
        const opDeleteRange = (ctx, op) => {
          if (op.ref == null) return { error: "delete_range requires a 'ref'" };
          const hit = resolveSetText(ctx, op.ref);
          if (!hit) return { error: `unresolved ref: ${op.ref}` };
          return { op: { op: "set_text", ref: hit.targetRef, text: "" } };
        };
        const opReplaceText = (ctx, op) => {
          ...
          if (op.ref != null) {
            ...
            const { text: next2, count: count2 } = ctx.replaceAllCounted(hit.el.text, query, replacement);
            return { op: { op: "set_text", ref: hit.targetRef, text: next2 }, replaced: count2 };
          }
          const target = elements.find((el2) => ctx.isSetTextTarget(el2.ref) && el2.text.includes(query));
        ...
        const OFFICE_OPS = new OfficeOpRegistry().define("replace_text", opReplaceText).define("set_cell", opSetCell).define("delete_range", opDeleteRange).define("insert_text", NATIVE).define("insert_paragraph", NATIVE).define("delete_paragraph", NATIVE).define("split", NATIVE).define("merge", NATIVE).define("insert_table", NATIVE).define("insert_table_row", NATIVE).define("delete_table_row", NATIVE).define("insert_table_column", NATIVE).define("delete_table_column", NATIVE).define("merge_cells", NATIVE).define("split_cell", NATIVE).define("insert_footnote", NATIVE).define("insert_endnote", NATIVE).define("insert_equation", NATIVE).define("set_columns", NATIVE).define("insert_picture", NATIVE).define("insert_slide", NATIVE).define("insert_shape", NATIVE).define("set_geometry", NATIVE).define("delete_node", NATIVE);
        function rewriteOfficeOp(ctx, op) {
          const handler = op && op.op ? OFFICE_OPS.registry[op.op] : void 0;
        ...
        function findOfficeReadMatch(elements, refs) {
        ...
        function slideRead(elements, slideRef, refString, target) {
          const prefix = slideRef + "/";
          const leaves = elements.filter(
            (el) => el.ref.startsWith(prefix) && ["text_frame", "cell", "shape"].includes(el.type) && String(el.text || "") !== ""
          );
        ...
        const DOCUMENT_SEARCH_COMMAND_EVENT = "ecrits:document-search-command"
        const DOCUMENT_SEARCH_RESULT_EVENT = "ecrits:document-search-result"
        const PREVIEW_DELTA_EVENT = "ecrits:editor-preview-delta"
        ...
        function documentElementPickerBridge() {
          return document.querySelector(PICKER_BRIDGE_SELECTOR)
        }

        function documentElementPickerEnabled() {
          return documentElementPickerBridge()?.dataset.enabled === "true"
        }

        function pickedElements() {
          try {
            return JSON.parse(documentElementPickerBridge()?.dataset.picks || "[]")
          } catch (_error) {
            return []
          }
        }
        ...
        function bindElementPickerTarget(target) {
          ...
          apply({ enabled: documentElementPickerEnabled(), picks: pickedElements() })
        ...
        //
        // This mirrors the HWP WASM browser adapter: the browser loads a
        // LibreOffice->WASM build and does
        ...
        const WasmOfficeEditor = {
          ...
          mounted() {
            ...
            // server also pushes `document.office.load_command` on re-open.
            this.handleEvent("document.office.load_command", (payload) => {
            ...
            // `document.engine.operation.command` -> handleAgentOp -> `document.engine.operation.replied` round-trip exactly;
          ...
          async loadDocument({ url, asset_version }) {
            ...
            // server's `document.office.load_command` push, which can race two concurrent
          ...
          buildPageStack() {
            ...
            for (const i of pageIndexes) {
              ...
              // Caret/adornment overlay (same backing-store size as the render canvas).
              // The blinking caret is drawn here so a caret blink never re-rasterizes the
              // page tile. Drag selections, find matches and picker adornments also paint
              // here because paintTile does not bake LOK's selection into the page RGBA.
          ...
          onCanvasMouseMove(event) {
            ...
            if (this.pickerEnabled() && !this.dragSelect) {
          ...
          // `ecrits:editor-state` bus the HWP editor uses so the toolbar lights
          ...
          emitToolbarState() {
            ...
            document.dispatchEvent(new CustomEvent("ecrits:editor-state", {
              detail: {
                document_id: this.documentId,
                ...
                // builds have no fontSizePt key → null here → the toolbar's finite>0
          ...
          updatePickerHover(event) {
            if (!this.pickerEnabled() || !this.api || !event) {
              this.setPickerHover(null)
              return
            }
            const loc = this.eventToPageLocal(event)
          ...
          updatePickerHoverAt(loc) {
            if (!this.pickerEnabled() || !this.api || !loc) {
              this.setPickerHover(null)
              return
            }
          ...
          pickerEnabled() {
            return documentElementPickerEnabled()
          },
          ...
          markViewerMutated(options = {}) {
            ...
            this.syncActiveOfficeReplayState()
            if (!this.documentId) return
            this.pushEvent("document.viewer.changed", { document_id: this.documentId })
          },
          ...
          // ── Find bar (⌘F) ──────────────────────────────────────────────────────────
          // The office arm drives LOK's OWN search: `.uno:ExecuteSearch` moves the
          // native document selection to the next/previous match. paintTile does NOT
          // include that selection, so after the command settles we paint a client
          // selection band from the selected text + native caret geometry. Search is
          // step-wise (no match enumeration), hence no n-of-m counter on this arm.
          handleFindCommand(detail) {
            // Mirrors (chat-rail previews) can share the viewed document's id — only
            // the authoritative viewer answers the find bar.
            if (this.mirror) return
            if (!this.activeToolbarTarget() || !this.toolbarCommandMatchesDocument(detail)) return
            ...
            switch (detail.action) {
              ...
              case "next":
              case "prev":
                this.queueOfficeFindAction(detail.action, query)
                break
              case "close":
                this.officeFindBar = null
                this.clearOfficeFindHighlight()
          ...
          // Find actions run SERIALIZED on a promise chain: `.uno:ExecuteSearch`
          // applies ASYNCHRONOUSLY (the LO main loop runs on its own pthread), so each
          // action must settle before the next one reads its pre-search snapshot.
          // Verdicting on the un-settled state is what produced the false "No matches"
          // on the initial search and the one-step-lagged caret/paint.
          queueOfficeFindAction(action, query) {
            if (query && !this.officeFindBar) {
              // The find SESSION starts at the first action after the bar opened:
              // anchor where the user was, so extending "c" → "ca" can re-match at
              // the same spot instead of skipping past the shorter match's selection.
              this.officeFindBar = { anchor: this.officeFindAnchorTwips() }
            }
            this._officeFindChain = (this._officeFindChain || Promise.resolve())
              .then(() => this.runOfficeFindAction(action, query))
              .catch(error => console.error("[office-wasm] find action failed", error))
          },

          // One find-bar action. `search` = the query text changed (search-as-you-type):
          // those searches re-run from the SESSION anchor. Steps (Enter/⇧Enter)
          // continue from the live selection, and wrap once around the document edge
          // when no further match moved the settled caret (recent LOK builds wrap on
          // their own — the manual retry covers builds/edges that don't).
          async runOfficeFindAction(action, query) {
            if (!query) {
              this.officeFindBar = null
              this.clearOfficeFindHighlight()
              this.emitOfficeFindState("", null)
              return
            }
            if (!this.officeFindBar) return // bar closed while this action was queued
            if (!this.hasApiMethod("postUnoCommand")) {
              this.emitOfficeFindState(query, null)
              return
            }
            const fresh = action === "search"
            const backward = action === "prev"

            const before = this.officeFindSnapshot()
            let result = await this.postFindSearch(query, backward, fresh ? this.officeFindBar.anchor : null, before)
            if (result && result.found && !fresh && before.caret && result.caret &&
                this.sameCaret(before.caret, result.caret)) {
              // The settled caret didn't move: either this is the only match, or there
              // is no further match in this direction (core keeps the previous
              // selection on a miss, which still equals the query). Wrap once from the
              // edge.
              result = await this.postFindSearch(query, backward, this.officeFindWrapAnchor(backward), {
                selection: result.selection,
                caret: result.caret
              })
            }
            if (!result || !this.officeFindBar) return // closed mid-settle
            this.officeFindBar.query = query
            if (result.found) {
              this.showOfficeFindHighlight(query, result.caret, backward)
              this.officeScrollCaretIntoView()
            } else {
              this.clearOfficeFindHighlight()
            }
            this.emitOfficeFindState(query, result.found ? null : 0)
          },

          // LOK exposes the selected text and the native caret, but this wasm build has
          // no LOK_CALLBACK_TEXT_SELECTION rectangle binding. Draw the missing visual
          // band on the existing selection overlay. Width uses caret height as the font
          // scale and character classes (CJK/full-width vs Latin/punctuation); it is
          // clamped to the page and remains visible even for a one-character match.
          showOfficeFindHighlight(query, caret, backward = false) {
            if (!caret || !Number.isFinite(Number(caret.page))) return
            const pageIndex = Math.max(0, Number(caret.page) - 1)
            const height = Math.max(12, Number(caret.height) || 16)
            const estimatedWidth = this.officeFindBandWidth(query, height)
            const caretX = Math.max(0, Number(caret.x) || 0)
            const pageRect = Array.isArray(this.pageRects) ? this.pageRects[pageIndex] : null
            const pageWidth = pageRect && Number(pageRect.w) > 0 ? Number(pageRect.w) / 15 : Infinity
            const width = backward
              ? Math.max(2, Math.min(estimatedWidth, Math.max(2, pageWidth - caretX)))
              : Math.max(2, Math.min(estimatedWidth, Math.max(2, caretX)))
            const x = backward ? caretX : Math.max(0, caretX - width)
            this.officeFindVisual = true
            this.setSelectionVisual([{
              pageIndex,
              x,
              y: Math.max(0, Number(caret.y) || 0),
              width,
              height
            }], true)
          },

          officeFindBandWidth(query, height) {
            let units = 0
            for (const char of Array.from(String(query || ""))) {
              if (/\s/u.test(char)) units += 0.34
              else if (/[^\u0000-\u02ff]/u.test(char)) units += 1.0
              else if (/[ilI1.,'`!|:;]/u.test(char)) units += 0.3
              else if (/[mwMW@#%&]/u.test(char)) units += 0.82
              else units += 0.56
            }
            return Math.max(height * 0.75, units * height * 0.82)
          },

          clearOfficeFindHighlight() {
            if (!this.officeFindVisual) return
            this.officeFindVisual = false
            this.clearSelectionVisual()
          },

          // Post one `.uno:ExecuteSearch` (SvxSearchItem Command 0 = FIND), wait for it
          // to APPLY, then reflect it — the repaint, the caret adopt and the found
          // verdict all run against the SETTLED state. Resolves { found, caret,
          // selection }, or null when the bar closed mid-settle. `found` = the settled
          // native selection IS the query (the only found-signal the wasm bindings
          // expose — there is no search-result callback export). Deliberately no
          // markViewerMutated: a search must not dirty the doc.
          async postFindSearch(query, backward, anchor, before) {
            const args = {
              "SearchItem.SearchString": { type: "string", value: query },
              "SearchItem.Backward": { type: "boolean", value: !!backward },
              "SearchItem.Command": { type: "long", value: 0 }
            }
            if (anchor) {
              args["SearchItem.SearchStartPointX"] = { type: "long", value: anchor.x }
              args["SearchItem.SearchStartPointY"] = { type: "long", value: anchor.y }
            }
            try {
              this.callApi("postUnoCommand", ".uno:ExecuteSearch", JSON.stringify(args))
            } catch (error) {
              console.error("[office-wasm] find ExecuteSearch failed", error)
              return { found: false, caret: before && before.caret, selection: before && before.selection }
            }
            const settled = await this.settleFindSearch(before)
            if (!settled) return null
            this.refreshCaret()
            this.renderAfterInput()
            return {
              found: this.officeFindSelectionMatches(settled.selection, query),
              caret: settled.caret,
              selection: settled.selection
            }
          },

          // Everything the found/wrap verdict compares, read LIVE from the bindings
          // (the cached this.caret lags behind an in-flight command).
          officeFindSnapshot() {
            let caret = null
            try {
              const res = this.callApi("getCursor")
              if (res && res.ok) caret = { page: res.page, x: res.x, y: res.y, height: res.height }
            } catch (_) {}
            return { selection: this.currentTextSelection(), caret }
          },

          // Poll (the same 40ms tick the caret settlers use) until the search visibly
          // applied — the native selection text or the cursor rect changed from the
          // pre-search snapshot — or the deadline passes: a genuine miss changes
          // NOTHING (core keeps the old selection), so it can only be recognized by
          // waiting it out. The first check runs synchronously, so a search that
          // applied in-call settles with zero delay. Resolves the settled snapshot, or
          // null when the bar closed mid-settle.
          settleFindSearch(before) {
            const tickMs = this.officeFindSettleTickMs ?? 40
            const maxTries = this.officeFindSettleMaxTries ?? 12
            return new Promise(resolve => {
              let tries = 0
              const tick = () => {
                if (!this.officeFindBar) return resolve(null)
                const snap = this.officeFindSnapshot()
                const beforeCaret = before ? before.caret : null
                const selectionChanged =
                  String(snap.selection || "") !== String((before && before.selection) || "")
                const caretChanged = !!snap.caret !== !!beforeCaret ||
                  !!(snap.caret && beforeCaret && !this.sameCaret(snap.caret, beforeCaret))
                if (selectionChanged || caretChanged || tries >= maxTries) return resolve(snap)
                tries++
                this.setOfficeTimer(tick, tickMs)
              }
              tick()
            })
          },

          // LOK search is case-insensitive by default and the selection can carry
          // newlines/padding for wrapped matches — compare whitespace-normalized,
          // case-folded text.
          officeFindSelectionMatches(selected, query) {
            const norm = value => String(value || "").replace(/\s+/g, " ").trim().toLowerCase()
            const selectedNorm = norm(selected)
            return !!selectedNorm && selectedNorm === norm(query)
          },

          // The caret's document position in twips (page-local logical px are twips/15,
          // page origins come from getPartPageRectangles) for SearchStartPointX/Y.
          // Null when unknown — the search then starts at the current LOK cursor.
          officeFindAnchorTwips() {
            const c = this.caret
            const rects = this.pageRects
            if (!c || !Array.isArray(rects) || rects.length === 0) return null
            const page = rects[Math.min(rects.length - 1, Math.max(0, (c.page || 1) - 1))]
            if (!page) return null
            return {
              x: Math.round(page.x + c.x * 15),
              y: Math.round(page.y + c.y * 15)
            }
          },

          // Manual wrap target: the document's very start (forward) or end (backward).
          officeFindWrapAnchor(backward) {
            if (!backward) return { x: 0, y: 0 }
            const rects = Array.isArray(this.pageRects) ? this.pageRects : []
            const last = rects[rects.length - 1]
            if (!last) return null
            return { x: last.x + last.w, y: last.y + last.h }
          },

          // Scroll the caret (at the found match) into the top third of the viewport
          // unless it is already visible. Impress search may have jumped slides with no
          // text-edit caret — fall back to the current part's page top.
          officeScrollCaretIntoView() {
            let pageIndex = null
            let y = 0
            if (this.caret && Number.isInteger(this.caret.page)) {
              pageIndex = this.caret.page - 1
              y = Number(this.caret.y || 0)
            } else {
              try {
                const part = this.callApi("getPart")
                if (Number.isInteger(part)) pageIndex = part
              } catch (_) {}
            }
            ...
            const top = Number(section.offsetTop || 0) + y * ratio
          ...
          // total: null = step-wise search (no counter), 0 = no match anywhere.
          emitOfficeFindState(query, total) {
            document.dispatchEvent(new CustomEvent(DOCUMENT_SEARCH_RESULT_EVENT, {
              detail: { document_id: this.documentId, query, total, index: null }
            }))
          },
          ...
          handleAgentOp({ request_id, verb, payload }) {
            if (this.mirror) return
            const reply = (body) => this.pushEvent("document.engine.operation.replied", { request_id, ...body })
          ...
          // Office-arm edit-op shim → colocated typed dispatch (#49 O4). The 3
        ...
        const syncCanvasState = hook => {
          const payload = JSON.parse(hook.el.dataset.canvasState || "{}")
          for (const [key, value] of Object.entries(payload)) {
            if (value == null) delete hook.el.dataset[key]
            else hook.el.dataset[key] = String(value)
          }
        }

        const OfficeWasmHook = {
          ...WasmOfficeEditor,
          mounted() {
            syncCanvasState(this)
            return WasmOfficeEditor.mounted.call(this)
          },
          updated() {
            syncCanvasState(this)
            return WasmOfficeEditor.updated.call(this)
          },
          destroyed() {
            return WasmOfficeEditor.destroyed.call(this)
          }
        }

        export default OfficeWasmHook
      </script>
    </div>
    """
  end
