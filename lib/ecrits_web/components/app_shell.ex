defmodule EcritsWeb.Components.AppShell do
  @moduledoc """
  Shared v33 Ecrits shell — the single global chrome wrapping the
  product surfaces (`PageController` `:home` / `/`, `StorageLive`,
  `DocumentLive`).

  See `docs/ecrits-studio-final-v33/SPEC.md` §4. The shell renders:

    * brand mark + "Ecrits" wordmark on the left, linking to `/`
    * topbar nav with the `문서들` link → `/storage`
    * a right-side account menu when the caller passes a signed-in
      `current_scope` (delegates to `EcritsWeb.Layouts.user_menu/1` so
  ...
  attr :current_scope, :map,
    default: nil,
    doc:
      "the current Ecrits.Context — pass through from the LiveView/conn so the topbar can render the account menu when signed in"
    ...
    <div class="min-h-screen pt-[60px] text-base-content bg-base-200">
      <header class="navbar fixed top-0 left-0 right-0 z-40 h-14 min-h-[60px] flex-nowrap border-b border-base-300 bg-base-200/90 supports-[backdrop-filter]:backdrop-blur-md px-7 max-md:px-4">
        <div class="navbar-start gap-6 min-w-0">
          ...
          <ul class="menu menu-horizontal p-0 text-[13px]" aria-label="Ecrits">
            <li>
              <.link
