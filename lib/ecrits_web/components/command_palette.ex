defmodule EcritsWeb.Components.CommandPalette do
  @moduledoc """
  Global Cmd+K / Ctrl+K command palette (Wave 3C0-B).

  Mounted once per authenticated page via `EcritsWeb.Layouts.app/1`. The
  ...
  Internally the palette is in one of:
    ...
    * `:search_law` — invokes `Ecrits.IO.LawMCP.search_law/2` and
  ...
  def mount_if_live(assigns) do
    cond do
      ...
      true ->
        ~H"""
        <.live_component
          module={EcritsWeb.Components.CommandPalette}
          id="cmd-k-palette"
  ...
  defp agent_help do
    ...
    ~H"""
    <p>
      The ecrits agent grades draft clauses, drafts replacement
