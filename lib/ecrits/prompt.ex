defmodule Ecrits.Prompt do
  ...
  defp mounted_vfs_preamble(_status, opts) do
    """
    [System] Stay in the user's scope and do not edit read-only requests. Open the document once with `doc.open_doc`; do not discover MCP resources. Use its ACP files like code: ACP read/search/edit for text and tables, while shell search stays read-only. The mounted file keeps one paragraph group per line — locate targets with line-based search and edit whole lines, preserving each line's trailing comma. Keep every document mutation in ACP; never use hwp5txt, raw-HWP extraction, or scripted shell rewrites. For brief-driven fills, account for every field, list item, and table row. Put one `미기재` in each unsupported blank while keeping its label, parentheses, and unit. Recheck counts, and reread the mounted file immediately before composing each write — the server may normalize between commits, and a write built from an earlier read is rejected as structural. Stage the complete set of supported text and table changes and commit them as ONE write — additional writes in the same turn may be unavailable. New content must be inserted as exactly one payload node per insert — a `table` node with `cells` (plus optional `header`) or a picture — added inside an existing paragraph group; new heading or paragraph nodes are structural and rejected with EINVAL. If a write is rejected with EINVAL, your staged bytes were probably built from a read older than the last commit: reread the mounted file and restage the same change from that fresh read. Only a requested native picture may use one post-commit `doc.find` and one image-only `doc.edit`; its marker must fit one rendered line, so keep it a few characters (an existing glyph like `(인)` made unique by its paragraph beats a long inline token), and when the exact target paragraph text repeats in the document, pass `occurrence` (1-based, document order) to pick the intended one.
    """ <> ultrathink_suffix(opts)
  end
