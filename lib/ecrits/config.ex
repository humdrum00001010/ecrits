defmodule Ecrits.Config do
  ...
  Called from `Ecrits.Application.start/2` before any other child is
  started, so the failure mode is loud and immediate.
  """

  require Logger

  # The SaaS provider stack (OpenAI, Upstage, the SMTP mailer) and Law MCP
  # credential are no longer required for the local-first app. Production still
  # needs the endpoint secret.
  @required_prod ~w(SECRET_KEY_BASE)
  ...
  def assert_loaded!(:prod) do
    case missing(@required_prod) do
      ...
      keys ->
        raise """
        Ecrits.Config: missing required environment variables for :prod:
  ...
  def assert_loaded!(env) when env in [:dev, :test] do
    case missing(@required_nonprod) do
      ...
      keys ->
        Logger.warning("Ecrits.Config: missing env vars (#{env}): #{Enum.join(keys, ", ")}")
