...
Before tracing, model the flow as ordered layers. Record each proposed layer, responsibility, and boundary as the agent's hypothesis rather than an established fact. Runtime evidence must mark each hypothesis as confirmed, refuted, or revised.

## Records

Maintain two concise tables:

1. A complete flow table that is the canonical source for a later test set. Record one row per observed hop with scenario and flow IDs, order, hypothesized layer and responsibility, hypothesis status, trigger and preconditions, boundary, full reproducible inputs and arguments, sync/async behavior, execution identity, timestamps and latency, memory before/after/delta, result, messages and state changes, side effects, lifecycle/free point, expected invariant, actual outcome, evidence source, and pass/fail status. Redact secrets without removing the structure needed to reproduce the flow.
