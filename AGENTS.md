...
- When the question is about how data/control *flows* (e.g. how the IR is exposed, how a value reaches a layer, what a function actually returns), **use `dbg` first, then look up code minimally.** Drive the real call through the live runtime with `project_eval` and `dbg/1`, observe the actual values at each hop, and only then open the source — and only the few lines needed to name the hop you already watched execute.
- The runtime is the source of truth for what passes through; the code is a secondary lookup to label what `dbg` already showed. Never lead with a code read, and never infer a flow's result from source when you can evaluate it.
...
LiveViews that require login should **always be placed inside the __existing__ `live_session :require_authenticated_user` block**:
    ...
    scope "/", AppWeb do
      ...
      live_session :require_authenticated_user,
        on_mount: [{EcritsWeb.UserAuth, :require_authenticated}] do
...
LiveViews that can work with or without authentication, **always use the __existing__ `:current_user` scope**, ie:
    ...
    scope "/", MyAppWeb do
      ...
      live_session :current_user,
        on_mount: [{EcritsWeb.UserAuth, :mount_current_scope}] do
