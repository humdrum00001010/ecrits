...
For exfuse bugs, work in `$EXFUSE_DIR`, test with `mise exec -- mix test`, push to `github.com/humdrum00001010/exfuse`, then run `mise exec -- mix deps.get` in ecrits — that is all. `mix deps.get` is aliased in `mix.exs` to `["deps.update exfuse phoenix_octet", "deps.get"]`, so it auto-advances both public branch-pinned repos to their just-pushed HEAD and rebuilds what changed (the trailing `deps.get` is the real task; Mix runs the underlying task, not this alias, so no recursion). Background: a plain unaliased `mix deps.get` would NOT pull a freshly pushed commit because git deps are SHA-pinned in `mix.lock`; the combined `deps.update` step re-resolves both repos in one Mix invocation. The other forks (`ehwp`, `libreofficex`) are NOT aliased, so they still need an explicit `mise exec -- mix deps.update <dep>` after their push.
