defmodule Contract.Local.Agent.EndpointTest do
  ...
  alias Contract.Local.Agent.Endpoint
  ...
  test "streams fake adapter tool events and executes local document read" do
    {:ok, session} =
      start_local_session(
        ...
        adapter_opts: [
          script: [
            {:tool_call, "positionalindex.read", %{"at" => 0, "size" => 1}},
            {:text_delta, "done"}
          ]
        ]
      )
  ...
  test "resolves document tools from session document id by default" do
    ...
    assert Enum.any?(tools, &(&1["namespace"] == "positionalindex" and &1["name"] == "read"))
  ...
  test "persists streamed events to opt-in JSONL" do
    path = Path.join(System.tmp_dir!(), "contract-local-agent-#{Ecto.UUID.generate()}.jsonl")
  ...
  defp open_local_document! do
    root =
      Path.join(
        System.tmp_dir!(),
        "contract-local-agent-doc-#{System.unique_integer([:positive])}"
      )
  ...
  defp codex_app_server_stub! do
    path =
      Path.join(System.tmp_dir!(), "contract-codex-app-server-stub-#{Ecto.UUID.generate()}.py")
