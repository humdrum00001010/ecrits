defmodule ContractWeb.LocalAgentAdapterStub do
  ...
  def stream_turn(turn, opts \\ []) do
    script = Keyword.get(opts, :script) || default_script(turn, opts)
  ...
  defp default_script(turn, opts) do
    script = [
      {:text_delta, "Test response: "},
      {:text_delta, input_text(turn.input)}
    ]

    if Keyword.get(opts, :echo_opts, false) do
      script ++ [{:text_delta, opts_text(turn, opts)}]
    else
      script
    end
  end

  defp opts_text(turn, opts) do
    [
      " model=#{Keyword.get(opts, :model)}",
      " reasoning=#{Keyword.get(opts, :reasoning_effort)}",
      " access=#{get_in(turn, [:tool_context, :access_control])}",
      " approval=#{Keyword.get(opts, :approval_policy)}",
      " sandbox=#{Keyword.get(opts, :sandbox)}",
      " permission=#{Keyword.get(opts, :permission_mode)}"
    ]
    |> Enum.join()
  end
  ...
  defp blocking_stream(script, message, opts) do
    ...
    Stream.resource(
      ...
      fn
        :waiting ->
          if is_pid(test_pid), do: send(test_pid, {:local_agent_adapter_waiting, self()})
